#include <weku/market_history/market_history_api.hpp>
...
{
   public:
      market_history_api_impl( weku::app::application& _app )
         :app( _app ) {}
...
order_book market_history_api_impl::get_order_book( uint32_t limit ) const
{
   ...
   const auto& order_idx = app.chain_database()->get_index< weku::chain::limit_order_index >().indices().get< weku::chain::by_price >();
...
market_history_api::market_history_api( const weku::app::api_context& ctx )
{
