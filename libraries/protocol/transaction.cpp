...
weku::protocol::transaction_id_type weku::protocol::transaction::id() const
{
   auto h = digest();
...
const signature_type& weku::protocol::signed_transaction::sign(const private_key_type& key, const chain_id_type& chain_id)
{
...
signature_type weku::protocol::signed_transaction::sign(const private_key_type& key, const chain_id_type& chain_id)const
{
   digest_type::encoder enc;
   fc::raw::pack( enc, chain_id );
   fc::raw::pack( enc, *this );
   ...
   ) const
{
   ...
   for( const public_key_type& k : s )
   {
      ...
      try
      {
         weku::protocol::verify_authority( operations, result, get_active, get_owner, get_posting, max_recursion );
...
{ try {
   weku::protocol::verify_authority( operations, get_signature_keys( chain_id ), get_active, get_owner, get_posting, max_recursion );
