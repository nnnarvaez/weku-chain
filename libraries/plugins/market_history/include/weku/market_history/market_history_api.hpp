...
#include <weku/market_history/market_history_plugin.hpp>
...
namespace weku{ namespace market_history {
...
{
   public:
      market_history_api( const weku::app::api_context& ctx );
...
FC_REFLECT( weku::market_history::market_ticker,
   (latest)(lowest_ask)(highest_bid)(percent_change)(steem_volume)(sbd_volume) );
FC_REFLECT( weku::market_history::market_volume,
   (steem_volume)(sbd_volume) );
FC_REFLECT( weku::market_history::order,
   (price)(steem)(sbd) );
FC_REFLECT( weku::market_history::order_book,
   (bids)(asks) );
FC_REFLECT( weku::market_history::market_trade,
   (date)(current_pays)(open_pays) );

FC_API( weku::market_history::market_history_api,
   (get_ticker)
   (get_volume)
   (get_order_book)
   (get_trade_history)
