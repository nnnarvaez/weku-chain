 ...
 */
        ...
        vector<tag_api_obj> database_api::get_trending_tags(string after, uint32_t limit, string community_name) const {
            return my->_db.with_read_lock([&]() {
                ...
                while (itr != ridx.end() && result.size() < limit) {
                    ...
                    /// TODO: need to refactory, since it's not a accurate way to filter
                    /// and get trending tags by community
                    /// here it just filters out the tags not belong to the community,
                    /// but the trending tags are not grouped and sorted by community yet.
                    ...
                    {
                        if(std::string(itr->community) == community_name)
                            result.push_back(tag_api_obj(*itr));
        ...
        vector<discussion> database_api::get_discussions(const discussion_query &query,
            ...
            while (count > 0 && tidx_itr != tidx.end()) {
                if (!query.community.empty() && query.community != std::string(tidx_itr->community)){
                    ++tidx_itr;
                    continue;
                }
                ...
                try {
                    ...
                    /*if (!query.community.empty()) {
                        string::size_type indexOfCommunityTag = result.back().json_metadata.find(query.community);
                        if (indexOfCommunityTag == string::npos) {
                            result.pop_back();
                            --itr_count;
                        }
                    } else  */
        ...
        vector<discussion> database_api::get_discussions_by_promoted(const discussion_query &query) const {
            return my->_db.with_read_lock([&]() {
                ...
                auto tidx_itr = tidx.lower_bound(boost::make_tuple(query.community, tag, parent, share_type(STEEMIT_MAX_SHARE_SUPPLY)));
        ...
        vector<discussion> database_api::get_discussions_by_trending(const discussion_query &query) const {
            ...
            return my->_db.with_read_lock([&]() {
                ...
                auto tidx_itr = tidx.lower_bound(boost::make_tuple(query.community, tag, parent, std::numeric_limits<double>::max()));
        ...
        vector<discussion> database_api::get_discussions_by_created(const discussion_query &query) const {
            return my->_db.with_read_lock([&]() {
                ...
                auto tidx_itr = tidx.lower_bound(boost::make_tuple(query.community, tag, parent, fc::time_point_sec::maximum()));
 ...
 */
        ...
        // this function is used to handle request from website
        state database_api::get_state(string path) const {
            return my->_db.with_read_lock([&]() {
                ...
                if (path.size() && path[path.size()] == '/')
                    path = path.substr(0, path.size()); /// remove '/' from end

                // Alexey: if website supports multiple communities, then the community name will be used.
                std::string community;
                if (path.size() && path[0] == '$'){
                    path = path.substr(1); // remove leading $ sign

                    string::size_type endOfCommunity = path.find("/");
                    if (endOfCommunity != string::npos){ // if something else after community name, ex: <community_name>/<tag>
                        community = path.substr(0, endOfCommunity);
                        path = path.substr(endOfCommunity + 1);
                    }
                    else // nothing after community name, ex: <community_name>
                        path = "";
                }
                ...
                try {
                    ...
                    // TODO: need to refactory, to get trending tags by community
                    /// FETCH CATEGORY STATE
                    auto trending_tags = get_trending_tags(std::string(), 50, community);
                    ...
                    // Alexey: user related urls, such as:
                    // /@username/feed, /@username/blog /@username/comments, /@username/transfers
                    // only used in user panel after login.
                    // ===== begin user panel =====
                    if (part[0].size() && part[0][0] == '@') {
                        ...
                        // used for reading a specific post
                        // ===== begin single post =====
                        // url example: /<category>/@<username>/<slug>
                    ...
                    //} else if (part[0].find("created") == 0) { // TODO: refoctory?? part[0] == "created"??
                    ...
                    } else if (part[0] == "recent") { // TODO: need refactory, merge with created??
                        discussion_query q;
                        q.tag = tag;
                        q.limit = 20;
                        q.truncate_body = 1024;
                        ...
                        //?? Alexey: need to find a way to filter tags for community if websites support it.
                        _state.tag_idx.trending.clear();
                        auto trending_tags = get_trending_tags(std::string(), 250, community);
