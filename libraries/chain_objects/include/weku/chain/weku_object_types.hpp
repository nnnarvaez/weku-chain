...
namespace weku { namespace chain 
{
   ...
   typedef oid< blacklist_vote_object                  > blacklist_vote_object_id_type;
...
{
   class variant;
   inline void to_variant( const weku::chain::shared_string& s, variant& var )
   {
      var = fc::string( weku::chain::to_string( s ) );
   }

   inline void from_variant( const variant& var, weku::chain::shared_string& s )
   {
   ...
   namespace raw
   {
      ...
      template< typename T > inline void pack( weku::chain::buffer_type& raw, const T& v )
      {
      ...
      template< typename T > inline void unpack( const weku::chain::buffer_type& raw, T& v )
      {
         datastream< const char* > ds( raw.data(), raw.size() );
         unpack( ds, v );
      }

      template< typename T > inline T unpack( const weku::chain::buffer_type& raw )
      {
         T v;
         datastream< const char* > ds( raw.data(), raw.size() );
         unpack( ds, v );
...
FC_REFLECT_TYPENAME( weku::chain::buffer_type )