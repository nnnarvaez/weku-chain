...
#include <weku/blockchain_statistics/blockchain_statistics_plugin.hpp>
...
{
   public:
      blockchain_statistics_api( const weku::app::api_context& ctx );
...
FC_REFLECT( weku::blockchain_statistics::statistics,
   (blocks)
   (bandwidth)
   (operations)
   (transactions)
...
FC_API( weku::blockchain_statistics::blockchain_statistics_api,
   (get_stats_for_time)
